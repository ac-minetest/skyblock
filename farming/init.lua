...
local restore_plant = function(pos)
...
minetest.register_node("farming:weed", {
	...
	-- rnd
	on_punch = function(pos, node, puncher, pointed_thing) restore_plant(pos) end,
...
function farming.plant_growth_timer(pos, elapsed, node_name)
	...
	-- small chance that plant turns into weed
	if math.random(100)<=10 then
		minetest.swap_node(pos,{name = "farming:weed"}); -- punch/activate weed to restore plant!
		local dmeta = minetest.get_meta(pos);
		dmeta:set_string("plant",node_name);
		dmeta:set_int("t",minetest.get_gametime())
		return
	end
...
dofile(farming.path.."/bananas.lua")
