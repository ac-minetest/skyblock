...
function furniture.stand(player, pos)
	local ppos = player:getpos(); if not ppos or not pos then return end -- rnd fix
	...
	if dist > 0.7 then
		...
		minetest.after(0.3, furniture.stand, player, pos) -- potential problem: player leaves, then what??
	end
end
...
local sit = function(pos, node)
	...
	for k,v in pairs(objs) do
		...
		if default.player_attached[name] ~= true then --keys.sneak == true and
			v:setpos(pos)
...
function furniture.register_wooden(name, def)
	...
	local update = function(pos, node, node_north, node_east, node_south, node_west)
		if node.param2 == 0 or node.param2 == 2 then
			...
			if connect == 0 then
				minetest.swap_node(pos, {name = "furniture:bench_"..subname, param2 = node.param2})
			elseif connect == 1 then
				if node.param2 == 0 then
					minetest.swap_node(pos, {name = "furniture:bench_left_"..subname, param2 = node.param2})
				else
					minetest.swap_node(pos, {name = "furniture:bench_right_"..subname, param2 = node.param2})
				end
	...
	local rotate = function(pos, node, new_param2)
		...
		if node_north.name == "furniture:bench_"..subname or node_north.name == "furniture:bench_middle_"..subname or
				node_north.name == "furniture:bench_right_"..subname or node_north.name == "furniture:bench_left_"..subname then
			update(north, node_north, false, false, new_node, false)
		end
		if node_east.name == "furniture:bench_"..subname or node_east.name == "furniture:bench_middle_"..subname or
				node_east.name == "furniture:bench_right_"..subname or node_east.name == "furniture:bench_left_"..subname then
			update(east, node_east, false, false, false, new_node)
		end
		if node_south.name == "furniture:bench_"..subname or node_south.name == "furniture:bench_middle_"..subname or
				node_south.name == "furniture:bench_right_"..subname or node_south.name == "furniture:bench_left_"..subname then
			update(south, node_south, new_node, false, false, false)
		end
		if node_west.name == "furniture:bench_"..subname or node_west.name == "furniture:bench_middle_"..subname or
				node_west.name == "furniture:bench_right_"..subname or node_west.name == "furniture:bench_left_"..subname then
	...
	minetest.register_node(":furniture:bench_middle_"..subname, {
		description = def.description_bench,
		tiles = def.tiles,
		paramtype = "light",
		paramtype2 = "facedir",
	...
	minetest.register_node(":furniture:bench_right_"..subname, {
		description = def.description_bench,
		tiles = def.tiles,
		paramtype = "light",
		paramtype2 = "facedir",
	...
	minetest.register_node(":furniture:bench_left_"..subname, {
		description = def.description_bench,
		tiles = def.tiles,
		paramtype = "light",
		paramtype2 = "facedir",
	...
	if def.handle_crafts ~= false then
		...
		minetest.register_craft({
			output = "furniture:bench_"..subname,
			recipe = {
